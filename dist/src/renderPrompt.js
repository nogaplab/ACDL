 ...
 */
export function renderPromptToHtml(prompt) {
    const titleHtml = renderPromptTitle(prompt.title);
    const bodyHtml = renderPromptBody(prompt.body);
...
function renderBracketedIndex(index) {
...
function renderContextVarBlock(block) {
    ...
    const rootIndexText = rootIndices.length > 0
        ...
        : "";
    segments.push(`<span class="segment base">${escapeHtml(block.base)}${rootIndexText}</span>`);
    // 2. Render every segment in the PathDesc chain.
    let current = block.path;
    while (current) {
        const segIndices = current.indices ?? [];
        const segIndexText = segIndices.length > 0
            ? segIndices.map(renderBracketedIndex).join("")
            : "";
        segments.push(`<span class="segment">${escapeHtml(current.base)}${segIndexText}</span>`);
...
function renderLoopOutsideRole(block) {
    ...
    const bodyHtml = (block.body ?? [])
        .map((child) => `<div class="loop-child">${renderTopLevelBlock(child)}</div>`)
        .join("\n");
    return `
...
function renderLoopInsideRole(block) {
    ...
    const header = `ForEach(${escapeHtml(indexText)}: ${escapeHtml(iterableText)}):`;
    const bodyHtml = (block.body ?? [])
        .map((child) => `<div class="role-loop-child">${renderRoleBuildingBlock(child)}</div>`)
        .join("\n");
    return `
...
function renderSwitchOutsideRole(block) {
    ...
    const casesHtml = (block.cases ?? [])
        ...
        const bodyHtml = (c.body ?? [])
            .map((child) => `<div class="switch-child">${renderTopLevelBlock(child)}</div>`)
            .join("\n");
        return `
...
function renderSwitchInsideRole(block) {
    ...
    const casesHtml = (block.cases ?? [])
        .map((c) => {
        const bodyHtml = (c.body ?? [])
            .map((child) => `<div class="role-switch-child">${renderRoleBuildingBlock(child)}</div>`)
            .join("\n");
        return `
<div class="switch-case">
  <div class="switch-case-header">Case "${escapeHtml(c.match)}":</div>
...
function renderConditionalInsideRole(block) {
    ...
    const ifBodyHtml = (block.IfBody ?? [])
        .map((child) => `<div class="role-condition-child">${renderRoleBuildingBlock(child)}</div>`)
        .join("\n");
    let fullHtml = `
...
function renderConditionalOutsideRole(block) {
    ...
    const ifHeader = `If (${escapeHtml(block.Ifcondition)}):`;
    const ifBodyHtml = (block.IfBody ?? [])
        .map((child /* PromptBlock */) => `<div class="conditional-child">${renderTopLevelBlock(child)}</div>`)
        .join("\n");
    let fullHtml = `
